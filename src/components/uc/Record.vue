...
export default {
  ...
  methods: {
    ...
    getList(pageNo) {
      ...
      this.$http
        .post(this.host + "/uc/asset/transaction_es", params)
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
            this.loading = false;
            if (resp.data) {
              let trueData = resp.data.hits;
  ...
  computed: {
    tableColumnsRecord() {
      ...
      columns.push({
        ...
        width: 160,
        render: (h, params) => {
          return h("div", {}, params.row._source.create_time);
        }
      });
      columns.push({
      ...
      columns.push({
        ...
        render(h, params) {
          return h(
            ...
            {
              attrs: {
                title: params.row._source.amount
              }
            },
            that.toFloor(params.row._source.amount || "0")
          );
        }
      });
      columns.push({
        ...
        render(h, params) {
          return h(
            ...
            {
              attrs: {
                title: params.row._source.fee
              }
            },
            that.toFloor(params.row._source.fee || "0")
          );
        }
      });
      columns.push({
        ...
        render(h, params) {
          return h(
            ...
            {
              attrs: {
                title: params.row._source.discount_fee
              }
            },
            that.toFloor(params.row._source.discount_fee || "0")
          );
        }
      });
      columns.push({
        ...
        render(h, params) {
          return h(
            ...
            {
              attrs: {
                title: params.row._source.real_fee
              }
            },
            that.toFloor(params.row._source.real_fee || "0")
          );
        }
      });
      columns.push({
