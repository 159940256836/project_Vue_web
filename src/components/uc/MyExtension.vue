    ...
    <div class="nav-right col-xs-12 col-md-10 padding-right-clear">
      ...
      <div class="bill_box rightarea padding-right-clear record account-box">
        ...
        <div class="message" v-if="this.changeActive==0">
          <Table stripe :columns="tablePromoteFriends" :data="dataPromoteFriends.data||[]" :loading="loading" :disabled-hover="true"></Table>
          <div class="page-wrap">
            <span>共{{dataPromoteFriends.totalElement}}条</span>
            <Page :current="currentPage" :total="parseInt(dataPromoteFriends.totalElement)" :page-size="pageSize" @on-change="promoteFriendsPageChange"></Page>
          </div>
        </div>
        <div class="message" v-else-if="this.changeActive==1">
          <Table stripe :columns="tablePromoteMoney" :page-size="pageSize" :data="dataPromoteMoney.data||[]" :loading="loading" :disabled-hover="true"></Table>
          <div class="page-wrap">
            <span>共{{dataPromoteMoney.totalElement}}条</span>
            <Page :current="currentPage" :total="parseInt(dataPromoteMoney.totalElement)" :page-size="pageSize" @on-change="promoteMoneyPageChange"></Page>
          </div>
        </div>
      </div>
    </div>
