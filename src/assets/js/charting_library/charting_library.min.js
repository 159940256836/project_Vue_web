...
var n = { mobile: { disabledFeatures: 'left_toolbar header_widget timeframes_toolbar edit_buttons_in_legend context_menus control_bar border_around_the_chart'.split(' '), enabledFeatures: [] }}, d = { BARS: 0, CANDLES: 1,
...
this.id = d.gId(); if (!a.datafeed) throw Error('Datafeed is not defined'); var b = { width: 800, height: 500, symbol: 'AA', interval: 'D', timeframe: null,
  ...
  settingsAdapter: a.settings_adapter }); this.options.time_frames = a.time_frames || b.time_frames; a.preset && (a = a.preset, n[a] ? (a = n[a], this.options.disabledFeatures = this.options.disabledFeatures.length>0 ? this.options.disabledFeatures.concat(a.disabledFeatures):a.disabledFeatures, this.options.enabledFeatures = this.options.enabledFeatures.length>0 ? this.options.enabledFeatures.concat(a.enabledFeatures):a.enabledFeatures):console.warn('Unknown preset: `' + a + '`')); this._ready_handlers = []; this.create()
 } }; d.widget.prototype =
{ _innerWindow: function() { return d.gEl(this.id).contentWindow }, _autoResizeChart: function() { this.options.fullscreen && (d.gEl(this.id).style.height = window.innerHeight + 'px') }, create: function() { 
...
window[this.options.uid] = { datafeed: this.options.datafeed, customFormatters: this.options.customFormatters, tradingController: this.options.tradingController, brokerFactory: this.options.brokerFactory,
...
b, c, d)
 }, removeChartFromServer: function(a, b) { this._innerWindow().tradingViewApi.removeChartFromServer(a, b) }, getIntervals: function() { return this._innerWindow().tradingViewApi.getIntervals() }, getStudiesList: function() { return this._innerWindow().tradingViewApi.getStudiesList() } }; g('onSymbolChange'); g('onIntervalChange'); [['onTick'], ['onAutoSaveNeeded'], ['onScreenshotReady'], ['onBarMarkClicked', 'onMarkClick'], ['onTimescaleMarkClicked', 'onTimescaleMarkClick']].forEach(function(a) { 
